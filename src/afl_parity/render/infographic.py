...
class Infographic:
    ...
    def create_infographic(self) -> None:
        ...
        if not self.traversal_output.first_hamiltonian_cycle:
            # explicit "do nothing" when there is no hamiltonian cycle to draw
            return

        self._draw_infographic()
    ...
    def _draw_infographic(self) -> None:
        ...
        for i, cur_winner in enumerate(hamiltonian_cycle):
            ...
            # typing allows 'none' to be a score for draws, but not possible in a hamiltonian cycle
            if not cur_game_deets.wscore:
                cur_winner_score: int = 0
            else:
                cur_winner_score: int = cur_game_deets.wscore
            if not cur_game_deets.lscore:
                cur_loser_score: int = 0
            else:
                cur_loser_score: int = cur_game_deets.lscore
        ...
        plt.savefig(
            ...
            / Path(f"hamiltonian_cycle_infographic_{self.season_results.season}.png")
        )
